...
const getFacultySchedule = async (req, res) => {
  ...
  try {
    const facultyData = await SCHEDULE.find({
      faculty: facultyId,
      semester: semId,
      isDeleted: false,
    });
...
const getSemFaculty = async (req, res) => {
  ...
  try {
    ...
    const semesterSchedules = await schedRawData(
      await SCHEDULE.find({ semester: semId }),
    );
    console.log("this", semesterSchedules);
    function getCoursesByFaculty(semesterScheds, facultyList) {
      const facultyMap = {};
      const facultyDetailsMap = {};
      facultyList.map((details) => {
        facultyMap[details._id.toString()] = [];
        facultyDetailsMap[details._id.toString()] = details;
      });
      ...
      for (const facultyId in facultyMap) {
        result.push({
          faculty: facultyDetailsMap[facultyId],
          schedules: facultyMap[facultyId],
        });
      }
...
const updateFaculty = async (req, res) => {
  const { firstName, lastName, employeeId, department, ALC, SLC, RLC, _id } =
    req.body;
  ...
  try {
    ...
    if (employeeId !== undefined) {
      ...
      updates.employeeId = String(employeeId).trim();
    }
    ...
    if (ALC !== undefined) {
      updates.ALC = ALC === "" ? 0 : Number(ALC);
    }

    if (SLC !== undefined) {
      updates.SLC = SLC === "" ? 0 : Number(SLC);
    }

    if (RLC !== undefined) {
      updates.RLC = RLC === "" ? 0 : Number(RLC);
    }
...
const createFaculty = async (req, res) => {
  const { firstName, lastName, employeeId, department } = req.body;

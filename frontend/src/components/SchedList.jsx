...
const SchedList = ({ editing, sched }) => {
    return (
        <table className="w-full h-24 border-separate border-spacing-0">
            ...
            <tbody>
                {sched && sched.map(({ course, faculty, room, schedule, section, students, _id }) => {
                    return (
                        <tr className="h-12 hover:bg-placebo-turquoise" key={_id}>
                            ...
                            <td className="border border-collapse border-enamelled-jewel border-b-1 border-x-0 border-t-0">{section}</td>
                            <td className="border border-collapse border-enamelled-jewel border-b-1 border-x-0 border-t-0">TBD</td>
                            <td className="border border-collapse border-enamelled-jewel border-b-1 border-x-0 border-t-0">{room.name}</td>
                            <td className="border border-collapse border-enamelled-jewel border-b-1 border-x-0 border-t-0">TBD</td>
                            <td className="border border-collapse border-enamelled-jewel border-b-1 border-x-0 border-t-0">{course.units}</td>
                            <td className="border border-collapse border-enamelled-jewel border-b-1 border-x-0 border-t-0">{students.map(({ name }) => { return name })}</td>

                                ...
                                semesterFaculties.length > 0 && semesterFaculties.map((faculty) => {
                                    ...
                                    return (
                                        <tr key={faculty._id} className="h-12 hover:bg-placebo-turquoise">
                                            ...
                                            <td className="border border-collapse border-black border-opacity-30 border-b-1 border-x-0 border-t-0">{faculty.load.ALC}</td>
                                            <td className="border border-collapse border-black border-opacity-30 border-b-1 border-x-0 border-t-0">{faculty.load.SLC}</td>
                                            <td className="border border-collapse border-black border-opacity-30 border-b-1 border-x-0 border-t-0">{faculty.load.RLC}</td>
                                            <td className="border border-collapse border-black border-opacity-30 border-b-1 border-x-0 border-t-0">{TLC + faculty.load.ALC + faculty.load.SLC + faculty.load.RLC}</td>
                                        </tr>
                                    )
                                })
                            }
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                    {
                    ...
                    }
                        {
                            ...
                            <div className="flex flex-col items-center justify-center">
                                ...
                                <button className="flex items-center justify-center font-semibold w-96 h-20 border text-xl text-enamelled-jewel border-enamelled-jewel rounded-md bg-placebo-turquoise transition ease-in duration-200 hover:shadow-custom">
